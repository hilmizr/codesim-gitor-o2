...
def get_code_graph_embedding(code_snippet, embed_dim=16):
    ...
    node2vec = Node2Vec(graph, dimensions=embed_dim, walk_length=30, num_walks=200, workers=4)
    ...
    for node in graph.nodes:
        if node in model.wv:
            ...
            embeddings[node] = [0] * embed_dim

    # Aggregate embeddings (mean) to represent the entire graph as a single vector
    graph_embedding = np.mean(list(embeddings.values()), axis=0).tolist()

    return graph_embedding, embeddings

def save_embeddings(file_path, graph_embedding, node_embeddings):
...
def calculate_similarity(code_snippet_1, code_snippet_2, embed_dim=16):
    ...
    code_embedding_1, node_embeddings_1 = get_code_graph_embedding(code_snippet_1, embed_dim=embed_dim)
    code_embedding_2, node_embeddings_2 = get_code_graph_embedding(code_snippet_2, embed_dim=embed_dim)

    # Save embeddings
    save_embeddings('embedding_1.json', code_embedding_1, node_embeddings_1)
    save_embeddings('embedding_2.json', code_embedding_2, node_embeddings_2)

    # Calculate Manhattan distance between the two embeddings.
    similarity = np.linalg.norm(np.array(code_embedding_1) - np.array(code_embedding_2), ord=1)
